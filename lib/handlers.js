...
handlers.tokensHandlers.post = function(data, callback)
{
    ...
    const afterRead = function(err, userData)
    {
        if(err){
        ...
        console.log('authenticated user', phone);

        // create a token
        const token = {
            'tokenId': 'asdfasdfasdfasdf', // helpers.createToken(20),
    ...
    dataStore.read('users', phone, afterRead);
};
