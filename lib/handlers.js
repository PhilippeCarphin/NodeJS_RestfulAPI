...
const tokenIsValid = function(tokenid, phone, callback){
    ...
    dataStore.read('tokens', tokenid, function(err, token){
        ...
        if(token.expires < Date.now()){
            // TODO Assess whether this is the right place to delete expired tokens.
...
handlers.checksHandlers.get = function(data, callback)
{
    checkid = 'checkid' in data.query ? data.query.checkid : false;

    if(!checkid){
        callback(400, {'Error': 'Request query must contain checkid'});
        return;
    }
    ...
    dataStore.read('checks', checkid, function(err, check){
        ...
        tokenid = 'tokenid' in data.headers ? data.headers.tokenid : false;
        phone = check.userPhone;
...
handlers.checksHandlers.delete = function(data, callback)
{
    checkid = 'checkid' in data.payload ? data.payload.checkid : false;
    ...
    dataStore.read('checks', checkid, function(err, check){
        ...
        if(!tokenid){
            callback(400, {'Error': 'Request header must contain tokenid'});
            return;
        }

        phone = 'userPhone' in check ? check.userPhone : false;

         if(!phone){
            callback(500, {'Error': 'Check with id ' + checkid + ' was found but does not contain a userPhone field'});
            return;
        }

        console.log('in checks:delete: tokenid:', tokenid);
        ...
        tokenIsValid(tokenid, phone, function(valid, extra) {
            ...
            dataStore.delete('checks', checkid, function(err){
                if(err){
                    callback(500, {'Error': 'Could not delete check with id ' + checkid});
                    return;
                }

                dataStore.read('users', phone, function(err, user){
                    if(err){
                        callback(500, {'Error': 'Could find user owning check with id ' + checkid});
                        return;
                    }

                    userChecks = user.checks;

                    checkIndex = userChecks.indexOf(checkid);
                    if(checkIndex < 0){
                        callback(500, {'Error': 'Could not remove deleted check from user\'s check list'});
                        return;
                    }
                    ...
                    dataStore.update('users', phone, user, function(err){
                        if(err)
                            callback(500, {'Error': 'Could not update user(' + phone + ') after deleting check with id ' + checkid});
                        else
                            callback(200, check);
                    });
                });
